 ...
 */
public final class TimeBatchViewRStream extends ViewSupport implements
		CloneableView, BatchingDataWindowView {
	// View parameters
	 ...
	 * Constructor.
	 * 
	 * @param msecIntervalSize
	 *            is the number of milliseconds to batch events for
	 * @param referencePoint
	 *            is the reference point onto which to base intervals, or null
	 *            if there is no such reference point supplied
	 * @param timeBatchViewFactory
	 *            fr copying this view in a group-by
	 * @param statementContext
	 *            is required view services
	 * @param forceOutput
	 *            is true if the batch should produce empty output if there is
	 *            no value to output following time intervals
	 * @param isStartEager
	 *            is true for start-eager
	 */
	public TimeBatchViewRStream(TimeBatchViewFactory timeBatchViewFactory,
			StatementContext statementContext, long msecIntervalSize,
			Long referencePoint, boolean forceOutput, boolean isStartEager) {
	...
	/**
	 * Gets the reference point to use to anchor interval start and end dates
	 * to.
	 * 
	...
	public final void update(EventBean[] newData, EventBean[] oldData) {
		if ((ExecutionPathDebugLog.isDebugEnabled) && (log.isDebugEnabled())) {
			log.debug(".update Received update, " + "  newData.length=="
		...
		// If we have an empty window about to be filled for the first time,
		// schedule a callback
		if (currentBatch.isEmpty()) {
			if (currentReferencePoint == null) {
				currentReferencePoint = initialReferencePoint;
				if (currentReferencePoint == null) {
					currentReferencePoint = statementContext
							.getSchedulingService().getTime();
				}
			}
	...
	/**
	 * This method updates child views and clears the batch of events. We
	 * schedule a new callback at this time if there were events in the batch.
	 */
	protected final void sendBatch() {
		isCallbackScheduled = false;

		if ((ExecutionPathDebugLog.isDebugEnabled) && (log.isDebugEnabled())) {
			log.debug(".sendBatch Update child views, " + "  time="
					+ statementContext.getSchedulingService().getTime());
		}

		// If there are child views and the batch was filled,
		// fireStatementStopped update method
		if (this.hasViews()) {
			// Convert to object arrays
			EventBean[] newData = null;
			EventBean[] oldData = null;
			if (!currentBatch.isEmpty()) {
				newData = currentBatch.toArray(new EventBean[currentBatch
						.size()]);
			}
		...
		if ((ExecutionPathDebugLog.isDebugEnabled) && (log.isDebugEnabled())) {
			log.debug(".sendBatch Published updated data, ....newData size="
					+ currentBatch.size());
		...
		// there have been any events in this or the last interval do we
		// schedule a callback,
	...
	public final String toString() {
		return this.getClass().getName() + " msecIntervalSize="
				+ msecIntervalSize + " initialReferencePoint="
	...
	private static final Log log = LogFactory
			.getLog(TimeBatchViewRStream.class);
}
