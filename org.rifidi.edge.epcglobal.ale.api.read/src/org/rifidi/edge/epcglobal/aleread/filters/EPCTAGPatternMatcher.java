 ...
 * 
 */
public class EPCTAGPatternMatcher extends AbstractPatternMatcher implements PatternMatcher {
	...
	public EPCTAGPatternMatcher(String input) {
		...
		if (mat.find()) {
			...
			for (String stuff : split) {
				...
				if (mat.find()) {
					matchers.add(new RangeFieldMatcher(Long.parseLong(mat
							.group(1)), Long.parseLong(mat.group(2))));
