 ...
 */
package org.rifidi.edge.regression.getproperty;
...
import org.rifidi.edge.regression.gettagreads.GetTagReads;
...
public class GetProperty {
	...
	private static final String dummyRI = "org.rifidi.edge.readerplugin.dummy.plugin.DummyReaderInfo";
	...
	public void testGetTagReads() {
		...
		PropertyConfiguration propConf = getProperty();
	...
	public PropertyConfiguration getProperty() {
		...
		propNames.add(testProperty);
		SessionGetPropertyCall call = new SessionGetPropertyCall(sessionSD,
				propNames);
		PropertyConfiguration pconf;
		try {
			pconf = call.makeCall();
			if (pconf == null) {
				Assert.fail("Property Configuration is null");
			} else {
				for (String string : pconf.getPropertyNames()) {
					logger.info(pconf.toString());
				}
