...
public class RegistryAutoRemoveTest {
	//private static final Log logger = LogFactory.getLog(RegistryAutoRemoveTest.class);
	...
	public void testRegistryAutoRemove(){
		...
		Assert.assertNotNull(connectionRegistryService.getAllReaderConnections());
		...
		for (ReaderConnection c :connectionRegistryService.getAllReaderConnections()){
			System.out.println(c.getAdapter());
		}
		
		Assert.assertTrue(connectionRegistryService.getAllReaderConnections().isEmpty());
		
	}
	

	@Inject
	public void setConnectionRegistryService(ReaderConnectionRegistryService regSer) {
