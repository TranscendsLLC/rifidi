 ...
 * 
 * This class is adapted from the one found at
...
public class CRC16 {
	...
	private static int[] table = { 0x0000, 0x1189, 0x2312, 0x329b, 0x4624,
	 ...
	 * 
	 * @param binaryString
	 *            A String containing ones and zeros
	 * @return The CRC16
	 */
	public static int calculateCRC(String hexString) {
		byte[] bytes = ConvertingUtil.toBytes(hexString);
		int crc = 0xFFFF;
		for (byte b : bytes) {
	...
	/**
	 * Returns the 16-bit CRC of the byte stream in data, using the specified
	 * start and end indeces. Because of limitations in the Java byte primitive
	 * type this method uses a char array to represent the argument byte stream
	 * and the two returned bytes. However the only allowed character values are
	 * 0x00 - 0xFF.
	 * 
	 * @param data
	 *            Source data for CRC calculation.
	 * @param iStartIndex
	 *            First index to use for CRC calculation, inclusive.
	 * @param iSize
	 *            Number of bytes to use for calculation.
	 * @param crc
	 *            Preload (seed) value of the CRC.
	 * @return CRC as char[2], LSByte first, where each char represents a value
	 *         0x00 - 0xFF (byte).
	 */
	public static char[] calculateCcittCrc(char[] data, int iStartIndex,
			int iSize, char crc) {
		if (iStartIndex < 0 || iStartIndex + iSize > data.length)
			throw new IllegalArgumentException(
					"The indices included in the CRC calculation are out of bounds for the input byte array."); //$NON-NLS-1$

		short index = 0;
		for (int count = iStartIndex; count < iStartIndex + iSize; ++count) {
			index = (byte) (crc ^ data[count]);
			if (index < 0)
				index += 256;
			crc = (char) ((crc >> 8) ^ table[index]);
		}

		crc = (char) (crc ^ ((char) 0xFFFF));
		char[] retVal = new char[2];

		retVal[0] = (char) (crc & 0xFF); // LSByte
		retVal[1] = (char) ((crc & 0xFF00) >> 8); // MSByte
		return retVal;
	}
